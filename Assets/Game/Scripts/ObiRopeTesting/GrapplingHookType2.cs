 ...
 */
public class GrapplingHookType2 : MonoBehaviour
{
    ...
    private void LaunchHook()
    {
        ...
        {
            launchedProjectile = Instantiate(ropeProjectile, character.transform.position, ropeProjectile.transform.rotation);
            MagicRopeProjectileLogic projectileLogic = launchedProjectile.GetComponent<MagicRopeProjectileLogic>();
    ...
    {
        if (attached == false && launched == true)
        {
            return;
        }
    ...
    void Update()
    {
        ...
        if (rope.isLoaded && attached == true)
        {
